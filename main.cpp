...
void ReadMNIST(std::string img_path, int num_img, int dim_img, std::vector<std::vector<double>> &img,
    ...
    std::ifstream img_file(img_path, std::ios::in | std::ios::binary);
    if (img_file.is_open()) {
        std::cout << "Images file " << img_path << std::endl;
        int header = 0;
        int count = 0;
        int n_rows = 0;
        int n_cols = 0;
    ...
    if (label_file.is_open()) {
        std::cout << "Label file " << label_path << std::endl;
        int header = 0;
        int count = 0;
...
void loadbatches(int numofbatch, int batch_size, std::vector<std::vector<double> > &batch_x,
                 std::vector<std::vector<double> > &batch_y,
                 std::vector<std::vector<double> > train_x, std::vector<std::vector<double> > train_y) {
    std::copy(train_x.begin() + batch_size * numofbatch, train_x.begin() + batch_size * (numofbatch + 1),
              batch_x.begin());
    std::copy(train_y.begin() + batch_size * numofbatch, train_y.begin() + batch_size * (numofbatch + 1),
              batch_y.begin());
}
...
int main() {
    ...
    int epochs = 10;
    int batch_size = 100;
    int num_train_data = 1000;
    int num_test_data = 100;
    std::vector<int> ls;
    int input = 0;


    std::cout << "Enter the <num_layer> (include input and output. eg, 4)" << std::endl;
    std::cin >> num_layer;
    std::cout << "Enter the <num_layer_neurons> (eg, 784 500 100 10)" << std::endl;
    for (int i = 0; i < num_layer; i++) {
        int size = 0;
        std::cin >> size;
        ls.push_back(size);
    }
    std::cout << "Enter the <learning_rate> <epochs> <batch_size> (eg, 0.1 10 128)" << std::endl;
    std::cin >> learning_rate >> epochs >> batch_size;
